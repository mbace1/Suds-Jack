...
const PRECACHE = [
  ...
  '../toko/js/signature.js?v=4',
  ...['surface', 'palette', 'face', 'util', 'glitch'].map(m => `../toko/js/${m}.js`),
  './', './index.html',
  ...['main', 'input', 'bullet', 'player', 'enemy', 'crowd', 'framing', 'arena', 'level', 'editor', 'audio', 'haptics', 'designer', 'lang', 'tuning', 'retro', 'specimen']
    .map(m => `./js/${m}.js?v=${TOKEN}`),
  ...['first-light', 'three-rings', 'boost-lane'].map(id => `./levels/${id}.json?v=${TOKEN}`),   // v239: the bundled levels (level.js BUNDLED)
