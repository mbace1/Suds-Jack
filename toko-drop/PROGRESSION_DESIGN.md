...
| 2 | Wave shape | **Pulses across the 20 s round** (the SMASH machinery, retargeted). **Shipped v256** — three fronts across a 12–15 s round. | shipped |
...
| — | What ends a round, once pulses spread across it? | *"3 likely but I would like to test 1"* — **empty-floor-only is the likely rule; test clock-only first.** **v256 ships both:** an empty floor after the last pulse ends it; otherwise the clock does, and survivors carry over. | shipped (both) |
...
**B2 (from `PLAYTEST_2026-09-17.md`), owner 2026-09-19:** *"Wave number. Waves can be
12–15 secs."* — escalation follows the wave number; rounds are 12–15 s. Shipped v256.

**Still to ask:** what campaign progress buys (Q13); difficulty tiers (Q14).
Q15 (what we measure) shipped in v256 as `restartGap` + `time`.
