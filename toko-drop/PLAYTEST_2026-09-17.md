...
- **The SLUG multiplied**: 36 slug bodies in 5 CC runs from far fewer
  spawns; one run logged **22 splits**; **43 bodies alive at wave 8.** The
  splits came from **bullets passing through a head into the body behind
  it** — the bot fires at the nearest segment from wherever it stands. And a
  3-segment slug hit in the middle becomes two **one-segment "slugs"** — a
  dome with an eye, which is not the design.
  *(Correction, v253: the first draft blamed a "dash-cut" — the DASH OFFENSE
  loop that cuts enemies exists only inside NEX DEUS. In the main game a dash
  has i-frames and slices bullets, and does not touch bodies.)*
...
at most three animals, not twenty-two. *(A "dash eats the segments it
crosses" rule was pitched here and built for v253 — then found to live only
inside NEX DEUS, whose dash cuts bodies; the main game's dash does not. It
stays as that cabinet's rule. A dash-through verb for the main game would be
a new decision, not a fix.)*
...
ceiling) → B1c+b (revenge) → T1+T2 (TORO reads) → P2/P1 (draw
