// Toko Live v32: compact conversation continuity driven by the user's thread.
...
const isNews=s=>/\b(news|headline|story|source|announcement|release|current events?|latest|today)\b/i.test(String(s||''));
...
function snapshot(){
  ...
  const lastYou=userTurns.at(-1)||previous.lastYou||'';
  const lastToko=tokoTurns.at(-1)||previous.lastToko||'';
  let project=null;
  for(let i=userTurns.length-1;i>=0;i--){const p=projectFrom(userTurns[i]);if(p){project=p;break}}
  // Keep a project for one short follow-up only when the user has not clearly changed subject.
  if(!project&&previous.project&&lastYou){const words=keywords(lastYou);const genericFollowup=/^(why|how|more|go on|continue|next|what next|explain|and|so|yes|no|exactly|actually)\b/i.test(lastYou.trim());if(genericFollowup||words.length<2)project=previous.project}
  const mode=isNews(lastYou)?'news':project?'project':'general';
  const subjects=keywords(userTurns.join(' '));
