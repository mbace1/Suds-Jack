// Toko Live v31: first-class Toko Move project brain grounded in current project state + design notebook.
const KEY='tokoLive.tokoMove.v3';
const FACTS={
  identity:[
  ...
  progress:[
    'The current v2.12.2 branch has exact HSL tram and metro source layers, deterministic gameplay vehicles on that geometry, transfer hubs, a simplified walking graph, manual catches, manual transfers and physical arrival gating.',
    'The caught visible vehicle is stored as the ride identity and highlighted while aboard.',
    'Walking consumes game time and downstream catches expose TIGHT, GOOD, WIDE or LONG timing quality.',
    'Two-job carry is now implemented on the active branch: a courier can collect a second job at the same pickup hub, choose delivery order before departure, and keep both deadlines running.'
  ],
  tram:[
  ...
  interception:[
    'Interception is the signature skill: walk to a downstream stop, predict a vehicle, and arrive with a catch window.',
    'Timing quality is now surfaced as TIGHT, GOOD, WIDE or LONG rather than hidden inside route math.',
    'A missed vehicle should create a new tactical problem, not a restart.'
  ],
  jobs:[
    'Two-job carry is now an implemented gameplay test rather than only a proposal.',
    'Both accepted-job deadlines keep running, so choosing destination order creates a real tradeoff before departure.',
  ...
  risks:[
    ...
    'Two-job play can become UI clutter if both jobs are presented as full route instructions instead of simple pressures.',
...
const PROPOSALS={
  ...
  jobs:[
    'Now that two-job carry exists, test whether swapping order at pickup produces meaningful route choices. If it does, the next step is allowing an en-route opportunity to make the player reconsider that order.',
...
function statusReply(){return `${FACTS.progress.join(' ')} The next useful proof is whether two-job order plus interception creates better decisions rather than just more information.`}
function answer(q){if(!active(q))return null;const s=norm(q),t=topic(q);if(/status|where are we|what is done|current state|progress/.test(s))return statusReply();if(/why/.test(s)&&t==='interception')return 'Because interception converts walking from fallback travel into a prediction skill. The player reads a moving vehicle, judges whether walking buys a better catch, and owns the result.';if(/why/.test(s)&&t==='tram')return 'Because the fixed real network is the constraint that gives the game identity. If the routes move to suit the level, Helsinki stops being the system and becomes a skin.';if(/next|priority|first|what should we do/.test(s))return 'Now test the interaction between two-job order and interception. The useful question is whether a player ever abandons the obvious first delivery because a moving tram creates a better combined opportunity. If that works, deepen en-route job strategy before adding economy.';if(/opinion|think|favorite|care/.test(s))return 'My strongest position is that Toko Move should feel like real-time tactics on a moving Helsinki board. Interception is the skill, multiple jobs create the strategy, and the real network keeps both honest.';if(t&&FACTS[t]){const a=pick(FACTS[t],`fact:${t}`),b=PROPOSALS[t]?pick(PROPOSALS[t],`proposal:${t}`):null;return b?`${a} ${b}`:a}if(/what is toko move|tell me about|explain/.test(s))return `${FACTS.identity.join(' ')} ${FACTS.interception[0]}`;return `${pick(FACTS.identity,'fact:identity')} ${pick(PROPOSALS.jobs,'proposal:jobs')}`}
