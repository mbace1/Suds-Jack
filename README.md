...
pixel — Toko's face, every panel, the Helsinki footage — is drawn in code.

Three channels on the dial: **87.60 KAIKU** (games / studios), **104.40
VERKKO** (tech / industry), **141.12 VARTIO** (defence / signal) — Helsinki as
what it actually is, a games town wired into a tech industry sitting next to a
defence band. Sixteen bulletins, ordered by band.

**The top frame is an edit, not a picture.** Every bulletin cuts on a beat
between the story **graphic**, **B-roll** of a low-poly Helsinki (drones holding
over the Esplanadi, the container harbour, the eastern treeline) and a **wide**
of the booth — three deliberately different registers, which is what makes a
post read as a cut package. DECODE cuts home to the graphic and holds, since
that is the only shot that decodes.

**Adding a bulletin is a JSON edit** — a roster entry and a copy block in each
of en/fi/ja in `wire.json`, then `node radiofree/tools/validate-wire.mjs`. No
build, no cache-token bump. `radiofree/STORIES.md` is the bar:
the register is The Onion — total deadpan, the joke in the fact and never in
the wording — and every plain reading has to be specific enough to be worth
...
**Trilingual fi / en / ja** — every word, bulletins included, and not
translations of each other's tricks: Finnish reaches for its true agentless
...
network — it is a feed you read on a metro.
...
tearing under DECODE. Seventy-two-check smoke gate in `radiofree/test/`, including the
...
### 2026-07
- **radiofree v6 — the border bulletin, and Toko moves house:** a thirteenth bulletin on the defence band — alliance-linked simulation money reshaping what the capital's studios build, and the eastern border talked about as a proving ground for counter-drone and cyber work. Its technique is **SPECULATION AS REPORTING**, the hedge stack: *could, describes as, is emerging, may, cannot be ruled out* — not one of them claims anything, and together they build a theatre you remember afterwards as reported fact. The panel does the same job and comes apart the same way: a map drawn full of tracks, cones and five contacts, which DECODE empties to the two that were actually observed. The actors stay unnamed, as on the rest of that band, which is what keeps the footer's "invented ministries" line true. Also from the owner's reference photo: **Toko now broadcasts from a flat rather than a studio** — a night kitchen behind her, a slack string of warm bulbs, and a cold laptop lighting her face from below, because one person on a sofa is the whole reason the station is pirate. The bulbs are the only warm light in the app that is not the decode, kept dim and desaturated so amber keeps its single job. The sign-off's count now comes from the roster instead of a hardcoded "twelve" in six strings
