...
### 2026-08
- **tiny2d v4 — a fat bird, and a difficulty curve that is not a wall:** the rider is now a **fat bird on a skateboard** (round mass, no neck, beak and tail carrying the silhouette at the ~40px a phone gives it; the arcade marquee follows). And the ramp that made deep runs frustrating turned out to be a *sharpness* ramp: amplitude climbed 4.5 → 9.5 while segments shortened 30 → 19, and a raised cosine peaks at `|dy|·π/(2·len)`, so doing both multiplied steepness — by 3 km the ground ran at 41° with 65° faces and a bot riding the line properly landed **100% hard**. Hills now come from a **slope budget** (26° → 39°), so a longer hill is a taller one and never a steeper one; faces run 42 → 36 units and the pop drops 14 → 10 (the ballistic range is v²/g ≈ 47 units, so a 28-unit face meant every real air overshot the trough); landing bands widen to 0.26 / 0.60 after measuring that the old 10° perfect window was unreachable by any line. Measured over ~370 airs per point: perfect **32% → 24%** from the start to 6 km, hard 33% → 42%. New `tiny2d/test/bench.cjs` runs the real physics headless at a fixed timestep and prints that curve
- **tiny2d v3 — the canvas was twice the size of the phone:** `resize()` passed `updateStyle: false` to `renderer.setSize()` and the canvas carried no CSS size, so the element laid out at its *drawing buffer* size in CSS pixels — with `setPixelRatio(min(dpr, 2))` that is **double the viewport in each direction on any phone**. The player saw the top-left quarter of the picture at 2×, with the skater (who rides the middle of it) below the bottom edge of the screen. Measured on a 393×851 screen at dpr 2: canvas 786×1702, skater at y=874; now 393×851 and y=481. `dropcabal` passes that flag deliberately — its CSS pins the canvas to 100vw/100vh — and this game had copied the flag without the CSS
- **tiny2d v2 — the phone can see the hill:** the ortho view is anchored on its **width** (60 units across at a standstill, 89 flat out) rather than its height, so the visible world no longer shrinks with the screen's aspect — a phone held upright was getting **16 units across**, less than one terrain segment, and rendered as featureless dark mass; it now gets 36–42, and 16:9 desktop is bit-for-bit the old view. The two constants still framed in world units came along: the sun is sized off the width (it had swollen to half a portrait screen) and the camera's forward lead is a fraction of the view rather than a flat 16 units (it had pinned the skater to the left edge). HUD fixes on top: readouts clear the arcade shell's HOME button, the ♪ toggle moves out from under the HOLD button on touchscreens, and the page's bare `button {}` rule is scoped to its own overlays — `min-width: 190px` had been leaking into the shell's HOLD button and stretching that circle into an ellipse
