...
### 2026-08
- **hyperdagger v4.38 — the pressure ceiling (measured, not guessed):** instrumented the real director over a full run and read what is actually *alive* rather than what the budget says — and the v4.36 curve had a cliff in it: minute one averaged 6 live threats, minute two averaged 41, and the late game climbed to 88 with nothing stopping it. The insight: **budget controls the spawn rate, but what kills you is the standing population**, and population runs away whenever spawns outpace kills — which is also a death spiral, since falling behind makes you fall further behind. The director now refuses to add pressure while the floor is at capacity (scaling ceiling, capped at 38; totems and eggs don't count — they're furniture). The first cut of this gated only the pulse filler and moved the measured population by *one body*: the real faucet turned out to be the **totem exhale**, six totems on a ~2 s cycle out-producing the entire pulse system. With that gated too, a firing player now sees **4.5 → 30 → plateau at 35** instead of 6 → 41 → 88-and-climbing. Debuts stay exempt — a new threat always gets its moment. 51-check gate, including that pressure *plateaus* rather than spirals
- **hyperdagger v4.37 — the arena:** the lighting and the ground stop reading as placeholder. The asset light rig was a generic studio key from above; it is now **motivated by the world**, which has exactly two light sources — the glowing grid floor and the ember horizon. The hemisphere is **inverted** (ground term = the white grid, sky term = the void), so imported meshes are lit *from the floor up* and a Meshy skull reads as standing in the arena instead of pasted into it; three crimson horizon lights ring the player, and both the uplight and the ember answer the same beat/trauma signals the floor does, so assets flush on damage exactly like the world. The floor itself is now **plates, not a wireframe**: recessed seams, a bevel that catches light on two sides and falls away on the other two, corner bolts, vent slots, per-plate variation — with the plates kept near-black so it is the *seam* that blooms, and the light reads as coming up through the floor rather than drawn on it. Plus an **`ARENA_ASSETS.floorPanel` slot**: register a Meshy floor tile and it is merged to one geometry, instanced across the disc, clipped to the rim and randomly quarter-turned, lit by the same rig as the enemies
- **hyperdagger v4.36 — the balance pass:** minute one is a **parade**, minute two is the **squeeze**. The first pulse now lands at t=10 and the cadence is ~9.5 s, so **six enemy types debut inside the first minute** (skulls, watcher, husk, brute, spider, blinker) plus totems and thorns — eight distinct threats where the old curve showed four — while the early budget rate is deliberately tiny, so each debut arrives nearly alone and per-pulse pressure in minute one actually *drops* (2.8–5.1 vs 3.8–9.0). Then the parade ends and the same cadence spends a budget climbing four times faster: **minute two carries ~61% more pressure**, the same enemies suddenly in numbers. The entire schedule moved into `tuning.js` (`T.director`) so the curve is one file, which immediately exposed a real bug — the heavy-pulse centrepiece gates still hardcoded the *old* serpent/dread unlock times and had silently disagreed with the pool ever since it was last retuned. 49-check gate, asserting the curve's shape so a future rebalance can move numbers but not flatten it
- **hyperdagger v4.35 — mesh asset integration:** the Nano Banana → Meshy pipeline lands. Register a GLB per enemy slot in `js/meshassets.js` and it takes over: the mesh rides in the hull slot as the **alive-skin** (lit by a new three-light rig that touches only imported assets — everything native stays unlit), and the lattice is **voxelized from the mesh at load** — surface rasterization with texture-color sampling, enclosed-interior fill, and the AO bake — so chips, chunk detachment, death bursts and the bone-yard keep working unchanged. When ~22% of the lattice is gone the skin **shatters off and the wounded voxel body fights on**. GLTFLoader vendored (dynamic import, paid for only when assets exist); everything fail-soft to the built-in string-art models. `ART_PIPELINE.md` carries the prompt sheet, `assets/README.md` the drop-in guide; 40-check gate
- **hyperdagger v4.34 — shotgun economy + offline gate:** the tap-burst gets Devil Daggers' actual economy — **the burst wins the moment, the stream wins the minute**. Burst count scales with weapon level (10/12/12/14) against a 0.8 s hand lockout, keeping burst DPS below stream DPS at every level so tap-spam never becomes the optimal close-range play; the smoke gate asserts the invariant arithmetically so no tuning pass can silently re-break it. Plus a real **offline gate** (`test/offline.cjs`): worker control, precache population, then the server dies, the browser goes offline, and the game must boot *and start a run* — with the arcade HOME shell as the one tolerated miss, since offline the hub it leads to is unreachable anyway. (v4.33 beneath it: idempotent enemy release caught by a new spawn/kill leak-plateau check — 33-check main gate)
- **hyperdagger v4.32 — the mesh hull:** enemies stop reading as cube stacks (the owner's verdict on the voxel look: "too Minecraft", settled with a rendered A/B/C test that showed denser subdivision changes almost nothing — the blockiness lives in the silhouette). While **alive**, every enemy now wears a **smoothed mesh skin** generated from its own voxel lattice: culled outer faces on welded corners, three passes of heavy Laplacian smoothing, one flat color per face so the HDR eyes stay crisp instead of smearing into the bone. The voxels stay the physics currency — **chips tear real holes and the skin re-forms around them**, severed chunks and deaths still burst genuine instanced-cube debris — so the matter identity survives with an organic read. The gauntlet and the blinker opt out (`noHull`): the checkerboard and the glitch shards *are* those designs. LOOK SMOOTH/CUBES toggle in the pause menu; the perf governor falls back to cubes on weak tiers; smoke gate at 31 checks
- **hyperdagger v4.31 — detail overhaul:** the skull family is a real **11×5×10 sculpt** now (357 source voxels, was 91) — jaw, two teeth rows, nasal cavity, flared cheekbones, deep eye sockets with the pupil recessed inside, brow ridge, dome — with serpent rings rebuilt as open armored hoops with a dorsal ridge, a serpent head with a dark maw and horns, and an armored lens on the watcher; `voxelSize` rescaled everywhere so hitboxes are unchanged. `parseModel` now **bakes shading**: neighbor-occlusion darkens crevices, sky exposure lifts tops, a hash grain breaks flat fills (HDR glow voxels pass through untouched). The neon got dialled back per the owner — edge rim halved, grid glow 1.8→1.5 — and the **serpent cruises as a deep sine in Y**, diving to the floor and arcing overhead instead of flying level. AUTO subdivision sits one tier lower since detail now comes from the sculpt; the smoke gate grows to 28 checks
- **hyperdagger v4.30 — HYPERDEMON visual push:** the other half of the homage gets its look. The sky's greyscale band shimmer becomes a **dim pastel iridescence** — three hue cycles drifting around the horizon, desaturated toward white, held at ≈0.09 peak so it stays atmosphere and never trips bloom — with the dark-red ember horizon still the danger line. The **grid itself glows** (`uGlow` 1.8 pushes the major lines past the bloom threshold, hotter on music beats). A new **neon rim pass** (cross-gradient edge detect, tinted by the STYLE accent, HDR) sits *before* the motion smear so every silhouette streaks and flares like the rest of the bright world; EDGE toggle in the pause menu, auto-shed below perf tier T1. And the **smear is dynamic**: damp 0.74 at rest rising to 0.86 mid-dash — HYPERDEMON smears hardest at speed, and the heavy end is always transient so the old 0.82-starburst trap stays closed. Four new smoke checks (23 total)
