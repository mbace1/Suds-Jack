// THE TEST CARD, IN THE PAGE (v2.61). The card is on the title with every
...
server.listen(0, '127.0.0.1', async () => {
  ...
  try {
    ...
    // a job, taken the way a thumb takes it
    await page.tap('#play'); await page.waitForTimeout(400);
    await page.tap('#jobBoard .jobOffer:not([disabled])');
    await page.waitForFunction(() => window.__tm.testcard.state.done.quiet, null, { timeout: 8000 }).catch(() => {});
    const feed1 = await page.evaluate(() => ({ done: { ...window.__tm.testcard.state.done }, feed: document.getElementById('feed')?.innerText || document.body.innerText }));
    ok('taking a job ticks "the quiet map"', !!feed1.done.quiet);
    ok('and the feed says so', /✓ TESTED · THE QUIET MAP · 1\/5/.test(feed1.feed), feed1.feed.slice(0, 200));
    ...
    await boot('?shift=3&day=none');
    const c1 = await card();
    ok(`and the card remembers after a reload (${c1.head})`, /2\/5/.test(c1.head) && c1.rows.filter(r => r.done).map(r => r.id).join() === 'quiet,mapboard');
