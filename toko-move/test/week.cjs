...
server.listen(0, '127.0.0.1', async () => {
  ...
  const ready = () => page.waitForFunction(() => window.__tm?.rival && !document.getElementById('play').disabled, null, { timeout: 30000 });
  ...
  try {
    ...
    // ── Monday, played out ───────────────────────────────────────────────
    await runOut();
    const end = await page.evaluate(() => ({ box: document.querySelector('.weekBox')?.innerText || '', next: document.getElementById('nextShift')?.textContent || '',
    ...
    ok(`it leads on to the next shift (${end.next})`, /TUESDAY/.test(end.next));
    ...
    await page.tap('#nextShift'); await ready();
    ...
    ok('and the page moves on to Wednesday rather than giving Tuesday back', wed.play === 'START WEDNESDAY' && wed.seed !== tueSeed);
    ...
    ok('opening the page without starting does not burn a day', w.day === 2);

    // ── to Friday ────────────────────────────────────────────────────────
    for (const d of ['WEDNESDAY', 'THURSDAY', 'FRIDAY']) {
      if (d !== 'WEDNESDAY') { await page.tap('#nextShift'); await ready(); }
      await runOut();
    }
    ...
    ok('the week is shared, not the shift', fri.share && !fri.next && fri.again);
    ...
    ok('the line has all five days and the rent', /MON .*€\d+/.test(text) && /FRI/.test(text) && /\(left\)/.test(text) && /rent|short/.test(text), text.replace(/\n/g, ' / '));
