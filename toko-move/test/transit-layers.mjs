...
const target = transit.layers.find(line => line.mode === 'TRAM');
assert.ok(target, 'tram layer exists');
transit.solo(target.id);
assert.deepEqual(transit.visibleLines().map(line => line.id), [target.id], 'a single tram can be soloed');
transit.toggle(target.id);
assert.equal(transit.visibleLines().length, 0, 'individual line can be hidden');
transit.showAll('TRAM');
assert.ok(transit.visibleLines().every(line => line.mode === 'TRAM'), 'tram-only view hides metro');
transit.showAll();
