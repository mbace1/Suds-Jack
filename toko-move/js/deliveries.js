...
export class DeliveryChallenge{
 ...
 acceptOffer(id){const job=this.offers.find(x=>x.id===id);if(!job)return{error:'job offer expired'};if(this.index>=this.target)return{error:'shift complete'};if(this.active){if(!this.canTakeSecond())return{error:'second job can only be collected before leaving this pickup hub'};this.queued={...job,originalStops:[...job.stops],acceptedAt:this.flow.clock.tick};this.offers=this.offers.filter(x=>x.id!==id);this.say(`CARRY 2 · ${CARGO[job.cargo]?.icon||'JOB'} · ${this.name(job.stops[1])} queued. Choose which destination to tackle first.`);return{job:this.queued,queued:true};}this.active={...job,originalStops:[...job.stops],acceptedAt:this.flow.clock.tick};this.offers=this.offers.filter(x=>x.id!==id);this.leg=0;this.startedAt=this.active.acceptedAt;this.launchLeg();const c=this.cargoRule();this.say(`${this.index+1}/${this.target} · ${c.icon} · ${this.routeLabel()} · ${c.rule}`);return{job:this.active};}
