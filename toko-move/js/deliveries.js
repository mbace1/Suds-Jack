// Toko Move v2.12 — courier dispatch board + physical HSL rides.
...
export class DeliveryChallenge{
 constructor(flow,say){this.flow=flow;this.say=say;this.index=0;this.leg=0;this.active=null;this.activeTrip=null;this.selectedPlan=null;this.waitingForCatch=false;this.seen=new Set;this.startedAt=0;this.score=0;this.late=0;this.bonuses=0;this.location='lasipalatsi';this.offers=[];this.offerCycle=0;}
 ...
 acceptOffer(id){if(this.active||this.index>=DELIVERY_TARGET)return{error:'finish the current job first'};const job=this.offers.find(x=>x.id===id);if(!job)return{error:'job offer expired'};this.active=job;this.offers=[];this.leg=0;this.startedAt=this.flow.clock.tick;this.launchLeg();const c=this.cargoRule();this.say(`${this.index+1}/${DELIVERY_TARGET} · ${c.icon} · ${this.routeLabel()} · ${c.rule}`);return{job};}
 ...
 catchChoice(choice){if(!this.active||!this.waitingForCatch||this.activeTrip||!choice?.legs?.length)return{error:'not waiting for a service'};const c=this.cargoRule();if(c.modes&&choice.legs.some(l=>!c.modes.includes(l.line.mode)))return{error:`${this.active.cargo} cannot use that service`};const legs=[];for(const leg of choice.legs){const route=this.runtimeRoute(leg.line);if(!route)return{error:`${leg.line.label} is not running on the gameplay network`};const fromIdx=route.indexOf(leg.from),toIdx=route.indexOf(leg.to);if(fromIdx<0||toIdx<0||fromIdx===toIdx)return{error:'that service does not cover the selected leg'};legs.push({routeId:route.id,from:leg.from,to:leg.to,fromIdx,toIdx});}const from=this.currentFrom(),to=choice.legs[choice.legs.length-1].to;const t=this.flow.inject(from,to,{kind:'delivery',job:this.index,leg:this.leg,label:this.active.label,cargo:this.active.cargo,n:1},{allowedModes:c.modes,urgency:c.express?2:1});t.legs=legs;t.legIdx=0;this.activeTrip=t;this.selectedPlan=choice;this.waitingForCatch=false;this.say(`CATCH · ${choice.legs.map(l=>l.line.label).join(' → ')} · ${this.name(from)} → ${this.name(to)}`);return{trip:t};}
 step(){for(const trip of this.flow.trips.completed){if(!trip.payload||trip.payload.kind!=='delivery'||this.seen.has(trip.id))continue;this.seen.add(trip.id);if(trip.payload.job!==this.index||trip.payload.leg!==this.leg)continue;const elapsed=this.elapsed(),c=this.cargoRule(),late=elapsed>this.active.limit;let earned=late?Math.round(this.active.value*.5):this.active.value,note=late?'LATE':'ON TIME';if(c.freshness){const freshLimit=Math.round(this.active.limit*c.freshness);if(elapsed<=freshLimit){const bonus=Math.round(this.active.value*.25);earned+=bonus;this.bonuses+=bonus;note='FRESH BONUS';}else if(!late){earned=Math.round(earned*.8);note='COOLED';}}if(c.fragile&&trip.legs?.length===1&&!late){earned+=35;this.bonuses+=35;note='FRAGILE SAFE';}if(c.express&&elapsed<=Math.round(this.active.limit*.7)){earned+=40;this.bonuses+=40;note='EXPRESS BONUS';}this.score+=earned;if(late)this.late++;this.location=this.currentTo();this.say(`${note} · +${earned} · ${this.active.label}`);this.index++;this.leg=0;this.active=null;this.activeTrip=null;this.selectedPlan=null;this.waitingForCatch=false;if(this.index<DELIVERY_TARGET)this.refreshOffers();return true;}return false;}
