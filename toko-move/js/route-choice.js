...
function arrivalState(tm,choice){const leg=choice?.legs?.[0],layer=leg&&layerFor(tm,leg.line);if(!layer||!tm.liveNetwork)return{ready:true,label:'READY',vehicle:null};const idx=nearestPathIndex(tm,layer,leg.from),dir=pathDirection(tm,layer,leg.from,leg.to),near=tm.liveNetwork.nearestTo(layer,idx,tm.flow.clock.tick,2.2,dir);if(near)return{ready:true,label:'now',eta:0,vehicle:near.vehicle};const eta=tm.liveNetwork.nextArrival(layer,idx,tm.flow.clock.tick,dir);return{ready:false,label:eta==null?'no service':tm.inFog?.(layer,eta)?'in the fog':inMinutes(eta,tm),eta,vehicle:null,fog:!!tm.inFog?.(layer,eta)};}
