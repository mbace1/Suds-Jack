...
// WHAT THE PARCEL MAY RIDE, applied BEFORE the top three are taken (v2.45).
// The panel took the three best plans and let the cargo rule refuse the tap:
// with a fragile parcel (tram only) at Kamppi, the three best were M1, M2 and a
// metro transfer, so every CATCH that lit was one the game would turn down and
// no tram was on offer at all. Found by playing the upcoming DAILIES: a
// sensible player stood at Kamppi for 2,800 of the shift's 3,000 ticks. One
...
function arrivalState(tm,choice){const leg=choice?.legs?.[0],layer=leg&&layerFor(tm,leg.line);if(!layer||!tm.liveNetwork)return{ready:true,label:'READY',vehicle:null};const idx=nearestPathIndex(tm,layer,leg.from),dir=pathDirection(tm,layer,leg.from,leg.to),near=tm.liveNetwork.nearestTo(layer,idx,tm.flow.clock.tick,2.2,dir);if(near&&tm.isFull?.(near.vehicle))return{ready:false,label:'full · wait for the next',eta:null,vehicle:null,full:true};if(near)return{ready:true,label:'now',eta:0,vehicle:near.vehicle};const eta=tm.liveNetwork.nextArrival(layer,idx,tm.flow.clock.tick,dir);return{ready:false,label:eta==null?'no service':tm.inFog?.(layer,eta)?'in the fog':inMinutes(eta,tm),eta,vehicle:null,fog:!!tm.inFog?.(layer,eta)};}
...
// THE MAP IS THE CONTROLLER (v2.50). The panel's buttons stay, but the same
// two decisions can be made where the trams are: a lit tram on the map is
// tapped to board it, and your stop is tapped to get off. Both go through the
// very calls the buttons make, so nothing about a catch depends on where the
// finger was.
export function catchables(tm){const st=tm.mobility?.status?.();if(st?.kind!=='waiting')return[];
 const cs=document.getElementById('routeChoices')?._choices||[];const out=[];
 for(const c of cs){const a=arrivalState(tm,c);if(a.ready&&a.vehicle&&!out.some(o=>o.vehicle.id===a.vehicle.id))out.push({choice:c,vehicle:a.vehicle});}return out;}
function wireMap(tm){if(tm._mapWired)return;tm._mapWired=true;
 tm.catchables=()=>catchables(tm);
 tm.catchVehicle=id=>{const hit=catchables(tm).find(x=>x.vehicle.id===id);if(!hit)return{error:'that one is not catchable'};const r=tm.challenge.catchChoice(hit.choice,hit.vehicle);if(!r?.error)render(tm,true);return r;};
 tm.getOffNow=()=>{const st=tm.mobility?.status?.();if(st?.kind!=='getoff')return{error:'not at your stop'};const r=tm.mobility.getOff();render(tm,true);return r;};}
function mount(){if(typeof window==='undefined')return;let last='';setInterval(()=>{const tm=window.__tm;if(!tm?.challenge)return;wireMap(tm);if(!document.body.classList.contains('transit-view'))tm.transit?.showAll?.();const k=structuralKey(tm);if(k!==last||!document.getElementById('routeChoices')){last=k;render(tm);}else refreshArrivals(tm);
