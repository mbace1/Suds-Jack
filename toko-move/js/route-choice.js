...
function chip(tm,leg){const layer=layerFor(tm,leg.line),colour=layer?.colour||'#52676d';return `<span style="display:inline-block;border-bottom:3px solid ${colour};background:#fffdf7;padding:3px 6px;margin:2px 4px 2px 0;font:700 11px ui-monospace,monospace"><b>${esc(leg.line.label)}</b> ${esc(leg.line.mode)} → ${esc(directionName(tm.city,leg))}</span>`;}
