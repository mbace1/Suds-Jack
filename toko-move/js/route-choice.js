...
function structuralKey(tm){const ch=tm.challenge,st=tm.mobility?.status?.();
  return ch?.active?`${ch.index}:${ch.leg}:${ch.currentFrom()}:${ch.currentTo()}:${ch.waitingForCatch}:${ch.activeTrip?.id||''}:${st?.kind||''}:${st?.remaining||''}:${st?.ride?.vehicleId||''}`:'done';}
