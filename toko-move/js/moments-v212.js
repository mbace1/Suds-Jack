...
export function mountSkillMoments(tm){
 ...
 const baseCatch=ch.catchChoice.bind(ch);ch.catchChoice=(choice,vehicle)=>{const r=baseCatch(choice,vehicle);if(r?.error)return r;const now=tm.flow.clock.tick,line=choice?.legs?.[0]?.line?.label||'service';if(walked&&ch.currentFrom?.()===walked.to&&now-walked.arriveTick>=0&&now-walked.arriveTick<=8){emit('INTERCEPTED',`${line} after walking · ${seconds(now-walked.arriveTick,tm)} margin`,{line,margin:now-walked.arriveTick});walked=null;}if(transferTick!=null&&now-transferTick<=6){emit('TIGHT CONNECTION',`${line} · ${seconds(now-transferTick,tm)} transfer`,{line,margin:now-transferTick});transferTick=null;}return r;};
