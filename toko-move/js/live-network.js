...
const SHIFT_HOURS = 16;                       // 06:00-22:00, the courier's day
export const END_TO_END_MINUTES = { TRAM: 50, SUBWAY: 45 };
export function speedFor(mode, ticksPerDay = 600) {
  const minutesPerTick = (SHIFT_HOURS * 60) / ticksPerDay;
...
export class LiveNetwork{
 constructor(transit,{vehiclesPerLine=2,dwellTicks=3,ticksPerDay=600}={}){this.transit=transit;this.vehiclesPerLine=vehiclesPerLine;this.dwellTicks=dwellTicks;this.vehicles=[];this.selectedVehicleId=null;for(const layer of transit?.layers||[]){if(layer.mode!=='TRAM'&&layer.mode!=='SUBWAY')continue;const count=layer.mode==='SUBWAY'?Math.max(2,vehiclesPerLine):vehiclesPerLine;for(let i=0;i<count;i++)this.vehicles.push({id:`${layer.id}:${i}`,layer,phase:(hash(layer.id)+i*997)%10000/10000,speed:speedFor(layer.mode,ticksPerDay)});}}
 ...
 nearestTo(layer,nodePathIndex,tick,maxTicks=2.2,direction=null){let best=null;for(const v of this.vehicles){if(v.layer.id!==layer.id)continue;const p=this.position(v,tick);if(!p||(direction&&p.direction!==direction))continue;const d=Math.abs(p.pathIndex-nodePathIndex)*this.ticksPerIndex(v);if(d<=maxTicks&&(!best||d<best.distance))best={vehicle:v,position:p,distance:d};}return best;}
