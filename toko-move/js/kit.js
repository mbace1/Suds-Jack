...
// on the HUD, a job's fee, the deadline, a regular's pips, the chain's
// multiplier, a drop's fee), so a kit is never a new thing to learn, only a
// reason to play the same thing differently.
//
// Three items were built, measured and CUT (v2.48): a bike (walking ×0.6),
// bubble wrap (a fragile parcel safe with a change) and a thermos (hot food
// fresh to the deadline). Each measured under €7 a week, because its lever is
// almost never pulled — the bot walked 2-3 times in twenty shifts, and hot or
// fragile work is a small share of the board. An item whose lever sits idle is
// a trap in an offer of three, so their levers were taken out with them.
//
...
// one place that lever lives (deliveries.js).
export const KIT=[
 {id:'bag',glyph:'🎒',name:'A BIGGER BAG',line:'Six units of room instead of five.',fx:{capacity:6}},
 {id:'contract',glyph:'📝',name:'A BETTER CONTRACT',line:'Every job pays a tenth more.',fx:{pay:1.1}},
 {id:'dispatch',glyph:'📟',name:'AN EASY DISPATCHER',line:'Every deadline is 15% longer.',fx:{limit:1.15}},
 {id:'cards',glyph:'📇',name:'BUSINESS CARDS',line:'Every regular knows you from the start: three pips.',fx:{known:3}},
 {id:'name',glyph:'⭐',name:'A GOOD NAME',line:'An on-time chain climbs ×0.3 a step, not ×0.25.',fx:{streak:0.3}},
...
export function effects(ids=[]){const fx={capacity:null,pay:1,limit:1,known:0,streak:null,drops:1};
 for(const id of ids){const k=BY_ID[id];if(!k)continue;const f=k.fx;
  ...
  if(f.pay)fx.pay*=f.pay;if(f.limit)fx.limit*=f.limit;if(f.known)fx.known=Math.max(fx.known,f.known);if(f.streak)fx.streak=Math.max(fx.streak||0,f.streak);if(f.drops)fx.drops*=f.drops;}
 return fx;}
