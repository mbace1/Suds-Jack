...
import './core-v212.js?v=38';
import './route-choice.js?v=14';
import {LiveNetwork,HEADWAY_MIN} from './live-network.js?v=9';
import {TRANSFER_HUBS,WALK_STREETS,walksFrom} from './hubs-walking.js?v=3';
import {MobilityController} from './mobility-v212.js?v=3';
import {interceptionOptions,bestInterception} from './interception-v212.js?v=2';
import {mountJobBoard,reachableSoon,planCost} from './job-board-v212.js?v=8';
...
const draw=()=>{tm.shiftLog?.poll();if(!document.body.classList.contains('transit-view')){drawInterception();const dpr=tm.renderer?.dpr||window.devicePixelRatio||1,filter=tm.fleetFilter?.();tm.trails?.update(ctx,tm.liveNetwork,tm.flow.clock.tick,project,dpr,filter);const rel=relevantLines(),boxes=tm.liveNetwork?.draw(ctx,tm.flow.clock.tick,project,dpr,{filter,priority:l=>rel.has(l?.name)||rel.has(l?.id)?2:1})||[];tm.drawStopLabels?.(boxes);drawCourier();rideStatus();}requestAnimationFrame(draw);};requestAnimationFrame(draw);}
mount();
