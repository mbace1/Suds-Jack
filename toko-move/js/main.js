// Toko Move v2.2 — Central Helsinki delivery challenge.
...
import { DeliveryChallenge, DELIVERY_TARGET } from './deliveries.js?v=4';
...
function boot(seed=7){
  ...
  drawer=new RouteDrawer($('map'),renderer,flow,{markersProvider:markers,onCommit:(mode,nodes)=>{const r=flow.addRoute(mode,nodes);say(r.error?`— ${r.error}`:`Your line: ${nodes.map(id=>flow.graph.node(id).name).join(' → ')}`);paintHud();},onTap:(hit,p)=>{if(hit.kind==='node'){sel=hit.id;paintSheet();hidePop();}else showPop(hit,p);},onDraft:d=>draft=d});
...
function popBody(hit){if(hit.kind!=='marker')return null;const t=hit.marker.data;if(!t)return null;return `<h3>${hit.marker.id.startsWith('pickup:')?'PICKUP':'DELIVER'}</h3><p>${esc(flow.graph.node(t.origin).name)} → ${esc(flow.graph.node(t.dest).name)}</p><p class="dim">${esc(t.payload?.cargo||'delivery')} · ${esc(t.payload?.label||'Delivery')}</p>`;}
function paintHud(){if(!challenge)return;$('done').textContent=`${challenge.index}/${DELIVERY_TARGET}`;$('reach').textContent=challenge.active?`${challenge.name(challenge.currentFrom())} → ${challenge.name(challenge.currentTo())}`:'complete';$('emit').textContent=challenge.active?`${challenge.remaining()}t`:`${challenge.score} pts`;$('clock').textContent=`${String(6+Math.floor(flow.clock.dayProgress*16)).padStart(2,'0')}:00`;$('lines').textContent=`${flow.routes.drawn.length}/${flow.routes.maxRoutes} lines`;}
function paintSheet(){const b=$('sheet');if(challenge?.active&&!sel){const j=challenge.active;const multi=j.stops.length>2?` · ${j.stops.length-1} legs`:'';b.innerHTML=`<h2>JOB ${challenge.index+1}/${DELIVERY_TARGET}</h2><p><b>${challenge.routeLabel()}</b></p><p class="hint">${j.label} · ${j.cargo}${multi}</p><p class="hint">Deadline ${j.limit} ticks · ${challenge.remaining()} remaining · score ${challenge.score}</p><p class="hint">Use Helsinki's existing network or drag between stops to add a connection.</p>`;return;}if(!sel){b.innerHTML='<p class="hint">Tap a stop for details.</p>';return;}const n=flow.graph.node(sel);b.innerHTML=`<h2>${n.name}</h2><p class="hint">${n.waiting.length} waiting · ${flow.routes.routesAt(sel).length} services call here</p><button class="btn wide" id="jobBack">SHOW CURRENT JOB</button>`;$('jobBack').onclick=()=>{sel=null;paintSheet();};}
...
function finish(){if(done)return;done=true;flow.clock.setPaused(true);$('endTitle').textContent=challenge.complete?'ALL DELIVERED':'DAY OVER';$('endStats').innerHTML=`<p>deliveries <b>${challenge.index}/${DELIVERY_TARGET}</b></p><p>score <b>${challenge.score}</b></p><p>late jobs <b>${challenge.late}</b></p>`;$('endNote').textContent=challenge.complete?(challenge.late?`All ten delivered. ${challenge.late} arrived late.`:'Perfect day: every job arrived on time.'):'Run it again and finish the route.';$('end').hidden=false;}
let last=0;function frame(now){const dt=last?Math.min(120,now-last):0;last=now;if(flow){flow.update(dt);renderer.draw(flow,{draft,alpha:flow.clock.alpha(),markers:markers()});if(flow.clock.tick%10===0){paintHud();if(!sel)paintSheet();}}requestAnimationFrame(frame);}
