// Toko Move v2.8 — delivery game on real HSL geography.
...
import { buildRealHelsinki } from './real-helsinki.js?v=1';

const BUILD_VERSION='2.7';
const $=id=>document.getElementById(id);
const MAP_THEME={...THEME,latent:THEME.paper,modeColours:{metro:'rgba(0,0,0,0)',tram:'rgba(0,0,0,0)',car:'rgba(0,0,0,0)'}};
let flow,challenge,renderer,drawer,draft=null,sel=null,done=false,msgs=[];
let transitLayers=null,transitView=false,realCity=null,sourcePack=null;
const say=s=>{msgs.unshift(s);msgs.length=Math.min(msgs.length,10);paintFeed();};

function boot(seed=7){
  if(!realCity)return;
  flow=createFlow({city:realCity,seed,days:1,demand:{rate:38,pairs:[{from:'home',to:'work',weight:2},{from:'transfer',to:'shop',weight:1}]},hooks:{onTick:()=>{if(challenge?.step()){paintHud();paintSheet();if(challenge.complete)finish();}},onDay:()=>finish(),onEvent:ev=>{if(ev.kind==='surge')say(`Crowding at ${flow.graph.node(ev.target).name}.`);}}});
  ...
  drawer?.destroy();renderer=new FlowRenderer($('map'),MAP_THEME);
...
async function init(){
  ...
  try{
    const r=await fetch('./cities/helsinki.json',{cache:'no-store'});if(!r.ok)throw new Error(`HSL pack ${r.status}`);
    sourcePack=await r.json();transitLayers=new TransitLayers(sourcePack);realCity=buildRealHelsinki(sourcePack);
    paintTransitPanel();boot();$('play').disabled=false;$('play').textContent='START SHIFT';requestAnimationFrame(frame);
  }catch(err){$('play').textContent='MAP LOAD FAILED';$('transitMeta').textContent=err.message;console.error(err);}
}
...
function showTransit(mode='all'){if(!transitLayers)return;transitView=true;document.body.classList.add('transit-view');$('transitPanel').hidden=false;if(mode==='TRAM')transitLayers.showAll('TRAM');else if(mode==='SUBWAY')transitLayers.showAll('SUBWAY');else transitLayers.showAll();paintTransitPanel();}
...
function drawExactTransitOnGameMap(){if(!transitLayers||!realCity||!flow)return;const c=$('map'),ctx=c.getContext('2d');transitLayers.draw(ctx,c.width,c.height,{fit:gameplayTransitFit,alpha:.46,lineWidth:1.5*(renderer?.dpr||1)});}
function drawTransitView(){if(!transitLayers)return;const c=$('map'),ctx=c.getContext('2d');ctx.save();ctx.fillStyle='#edf4f2';ctx.fillRect(0,0,c.width,c.height);ctx.restore();transitLayers.draw(ctx,c.width,c.height,{alpha:.95,lineWidth:2.6*(renderer?.dpr||1)});const p=transitLayers.source,d=renderer?.dpr||1;ctx.save();ctx.fillStyle='#233d4d';ctx.globalAlpha=.75;ctx.font=`${Math.round(10*d)}px ui-monospace,monospace`;ctx.fillText(`TOKO MOVE v${BUILD_VERSION} · REAL HSL GEOMETRY`,12*d,18*d);ctx.fillStyle='#647176';ctx.globalAlpha=.7;ctx.font=`${Math.round(8*d)}px ui-monospace,monospace`;ctx.fillText(`feed ${p.fetched} · ${p.clippedTo?'central extract':'full Helsinki pack'}`,12*d,31*d);ctx.restore();}
...
let last=0;function frame(now){const dt=last?Math.min(120,now-last):0;last=now;if(flow){flow.update(dt);if(transitView)drawTransitView();else{renderer.draw(flow,{draft,alpha:flow.clock.alpha(),markers:markers()});drawExactTransitOnGameMap();drawCityOverlay();}if(flow.clock.tick%10===0){paintHud();if(!sel)paintSheet();}}requestAnimationFrame(frame);}
addEventListener('resize',()=>renderer?.resize());$('play').onclick=()=>{if(!flow)return;$('title').hidden=true;flow.clock.setPaused(false);};$('pause').onclick=()=>{if(!flow)return;flow.clock.setPaused(!flow.clock.paused);$('pause').textContent=flow.clock.paused?'▶':'❚❚';};$('speed').onclick=()=>{if(!flow)return;const s=flow.clock.speed>=4?1:flow.clock.speed*2;flow.clock.setSpeed(s);$('speed').textContent=`×${s}`;};$('again').onclick=()=>{$('end').hidden=true;boot(7);flow.clock.setPaused(false);};$('popClose').onclick=hidePop;
$('transit').onclick=()=>transitView?hideTransit():showTransit('all');$('transitClose').onclick=hideTransit;$('tramOnly').onclick=()=>showTransit('TRAM');$('metroOnly').onclick=()=>showTransit('SUBWAY');$('allTransit').onclick=()=>showTransit('all');addEventListener('keydown',e=>{if(e.key==='Escape'){if(transitView)hideTransit();else hidePop();}});
init();window.__tm={version:BUILD_VERSION,get flow(){return flow;},get challenge(){return challenge;},get transit(){return transitLayers;},get city(){return realCity;},debug:{boot,finish,markers,showTransit,hideTransit}};
