// Toko Move v2.7 — delivery game + exact HSL transit inspector.
...
function coverageLabel(source){
  if(source.clippedTo)return `exact inside ${source.clippedTo.s}–${source.clippedTo.n} N`;
  return 'full Helsinki source pack';
}
function paintTransitPanel(){
  if(!transitLayers)return;
  const s=transitLayers.source;
  $('transitMeta').textContent=`${s.source} · ${s.fetched} · ${transitLayers.layers.length} source lines · ${coverageLabel(s)}`;
...
function drawTransitView(){
  ...
  ctx.save();ctx.fillStyle='#233d4d';ctx.globalAlpha=.75;ctx.font=`${Math.round(10*d)}px ui-monospace,monospace`;ctx.fillText(`TOKO MOVE v${BUILD_VERSION} · REAL HSL GEOMETRY`,12*d,18*d);ctx.fillStyle='#647176';ctx.globalAlpha=.7;ctx.font=`${Math.round(8*d)}px ui-monospace,monospace`;ctx.fillText(`feed ${p.fetched} · ${p.clippedTo?'central extract':'full Helsinki pack'}`,12*d,31*d);ctx.restore();
}
