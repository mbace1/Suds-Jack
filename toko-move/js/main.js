// Toko Move v2.3 — Central Helsinki cargo challenge + stronger city read.
...
function boot(seed=7){
  ...
  drawer=new RouteDrawer($('map'),renderer,flow,{markersProvider:markers,onCommit:(mode,nodes)=>{const r=flow.addRoute(mode,nodes);say(r.error?`— ${r.error}`:`NEW LINE · ${nodes.map(id=>flow.graph.node(id).name).join(' → ')}`);paintHud();},onTap:(hit,p)=>{if(hit.kind==='node'){sel=hit.id;paintSheet();hidePop();}else showPop(hit,p);},onDraft:d=>draft=d});
...
function markers(){
  ...
  for(const t of flow.trips.active){if(t.payload?.kind!=='delivery')continue;const col=cargoColour(t.payload.cargo);out.push({id:`job:${t.id}`,node:t.dest,slot:slot(t.dest),glyph:'▣',color:col,data:t});out.push({id:`pickup:${t.id}`,node:t.origin,slot:slot(t.origin),glyph:'●',color:'#2f9fb8',data:t});}
  return out;
}
...
function drawCityOverlay(){
  const c=$('map'),ctx=c.getContext('2d'); if(!flow||!renderer)return;
  const f=flow.graph.fit(c.width,c.height),d=renderer.dpr||1;
  ctx.save();ctx.globalAlpha=.12;ctx.fillStyle='#2f9fb8';
  ctx.beginPath();ctx.moveTo(0,c.height*.82);ctx.lineTo(c.width,c.height*.74);ctx.lineTo(c.width,c.height);ctx.lineTo(0,c.height);ctx.closePath();ctx.fill();
  ctx.globalAlpha=.16;ctx.strokeStyle='#2f9fb8';ctx.lineWidth=1.2*d;ctx.setLineDash([5*d,6*d]);ctx.beginPath();ctx.moveTo(f.x(58),f.y(78));ctx.lineTo(f.x(72),f.y(78));ctx.lineTo(f.x(88),f.y(82));ctx.stroke();ctx.setLineDash([]);
  ctx.globalAlpha=.52;ctx.fillStyle='#5d6a72';ctx.font=`${Math.round(9*d)}px ui-monospace,monospace`;ctx.textAlign='center';
  for(const [id,label] of [['pasila','PASILA'],['rautatientori','CITY CENTRE'],['kalasatama','KALASATAMA'],['katajanokka','HARBOUR']]){const n=flow.graph.node(id);if(n)ctx.fillText(label,f.x(n.x),f.y(n.y)-20*d);}
  ctx.restore();
}
...
let last=0;function frame(now){const dt=last?Math.min(120,now-last):0;last=now;if(flow){flow.update(dt);renderer.draw(flow,{draft,alpha:flow.clock.alpha(),markers:markers()});drawCityOverlay();if(flow.clock.tick%10===0){paintHud();if(!sel)paintSheet();}}requestAnimationFrame(frame);}
