...
export class TransitLayers {
  constructor(pack) {
    this.pack=pack;
    this.layers=pack.lines.map((line,index)=>({id:line.id,name:line.name,mode:line.mode,colour:line.mode==='SUBWAY'?'#ff6319':colourFor(index),path:line.path,visible:true}));
  ...
  draw(ctx,width,height,{fit=null,alpha=.9,lineWidth=3}={}){
    const project=fit||bboxFit(this.bounds,width,height);ctx.save();ctx.globalAlpha=alpha;ctx.lineCap='round';ctx.lineJoin='round';
    for(const layer of this.layers){if(!layer.visible||!Array.isArray(layer.path)||layer.path.length<2)continue;ctx.strokeStyle=layer.colour;ctx.lineWidth=layer.mode==='SUBWAY'?lineWidth*1.45:lineWidth;ctx.beginPath();for(let i=0;i<layer.path.length;i++){const[lat,lon]=layer.path[i],p=project(lat,lon);i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);}ctx.stroke();}
    ctx.restore();
  }
}

export function boundsFromLines(lines){
  let s=Infinity,n=-Infinity,w=Infinity,e=-Infinity;
  for(const line of lines)for(const p of line.path||[]){const[lat,lon]=p;if(!Number.isFinite(lat)||!Number.isFinite(lon))continue;s=Math.min(s,lat);n=Math.max(n,lat);w=Math.min(w,lon);e=Math.max(e,lon);}
  if(!Number.isFinite(s))throw new Error('Transit pack has no finite source geometry');
  return{s,n,w,e};
}
