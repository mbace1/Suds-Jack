...
import { sceneState, drawSceneState } from './sceneweather.js?v=58';
import { drawPassersby, drawBroadcastFX } from './broadcastfx.js?v=54';

export const AMBIENT_KEYS = ['metro','mannerheimrain','centralstation','hakaniemi','katajanokka','rooftops','kallionight','pasila','merihaka','toolo'];
const W=128,H=152;
...
function pasila(scr,t,d){drawPasila(scr,t,d);drawPassersby(scr,t,d,120);}
function merihaka(scr,t,d){drawMerihaka(scr,t,d);drawPassersby(scr,t,d,121);}
function toolo(scr,t,d){drawToolo(scr,t,d);drawPassersby(scr,t,d,122);}
const SCENES={metro,mannerheimrain,centralstation,hakaniemi,katajanokka,rooftops,kallionight,pasila,merihaka,toolo};
export function drawAmbient(key,scr,t,decode=0){const fn=SCENES[key]||rooftops,d=Math.min(.28,decode*.28);fn(scr,t,d);const seed=AMBIENT_KEYS.indexOf(key)+Math.floor(t/24);drawSceneState(scr,t,d,sceneState(seed));drawBroadcastFX(scr,t,decode);scr.scanlines(PAL.INK,3);}
