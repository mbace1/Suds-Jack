...
import { drawAmbient, AMBIENT_KEYS } from './ambient.js?v=46';
...
class AmbientFootage {
  constructor(host, story, seed = 0) {
    ...
    this.t = seed * 1.37;
    this.live = false;
    this.decoded = false; // compatibility only; decode presentation is retired.
...
export class Package {
  ...
  get decoded() { return false; }
  set decoded(v) { this._decoded = false; }
  ...
  update(dt, mouth = 0) {
    ...
    if (this.flashT > 0) {
      ...
      this.flash.style.opacity = String((this.flashT / CUT_FLASH) * 0.42);
    }
    if (!this.live) return;
    this.clock += dt;
