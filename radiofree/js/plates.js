...
import { mix, shade, bayer } from './screen.js?v=13';
...
function cathedralBase(c) {
  ...
  P(c, 0, 270, W, H - 270, '#0d1512');                          // the square
...
function cathedral(scr, t, d) {
  scr.ctx.drawImage(base('cathedral', cathedralBase), 0, 0);
  const c = scr.ctx;
...
function katuBase(c) {
  ...
  for (const [tx, ty, r] of [[38, 176, 13], [104, 174, 14], [52, 168, 8], [92, 166, 9]]) {
...
function katu(scr, t, d) {
  scr.ctx.drawImage(base('katu', katuBase), 0, 0);
  const c = scr.ctx;
  ...
  const app = (t * 0.05) % 1;
  const sc = 0.85 + app * 0.95;        // it arrives, rather than staying a dot
  const ty = 166 + app * 40;
...
function mannerheimBase(c) {
  ...
  const VX = 72, VY = 160;
  for (const [off, col, wdt] of [[-26, '#5e8a6f', 2], [-14, '#7fae8d', 1], [-5, '#9fc9ac', 2],
...
function mannerheim(scr, t, d) {
  scr.ctx.drawImage(base('mannerheim', mannerheimBase), 0, 0);
  const c = scr.ctx;
