...
export const seed = {
  ...
  start(host, ctx) {
    ...
    function drawSky(dayPhase, now = 0) {
      // simple day / night sky above the soil — never a still one: waiting is
      // the whole subject here, and waiting has weather in it
      if (dayPhase < 0.5) {
        // day
        scr.bands(0, 0, 192, 96, [PAL.SKY_DAY_TOP, '#93bfdd', PAL.SKY_DAY_LOW]);
        scr.disc(148, 22, 7, PAL.SUN, true);
        scr.px((now / 210) % 240 - 40, 30, 34, 4, '#c8dcea');        // a slow cloud
        scr.px((now / 170) % 250 - 50, 42, 20, 3, '#b6cee0');
        for (let i = 0; i < 5; i++) {                                 // motes in the light
          scr.px(40 + (i * 31) % 120 + Math.sin(now / 500 + i * 2) * 4,
                 56 + (i * 9) % 26 + Math.cos(now / 420 + i) * 3, 1, 1, '#dcecd8');
        }
      } else {
        // night
        scr.bands(0, 0, 192, 96, ['#0a0c18', '#0e1220', '#131828']);
        // a few stars, breathing
        for (let i = 0; i < 11; i++) {
          const x = (i * 37 + 9) % 180;
          const y = 8 + (i * 13) % 50;
          const tw = Math.sin(now / 520 + i * 2.1);
          if (tw < -0.5) continue;
          scr.px(x, y, 1, 1, tw > 0.65 ? '#c8d0e8' : '#8a90a8');
        }
      }
    }
