// Kindling v8 breeding / lineage smoke gate.
...
(async () => {
  ...
  // Give the discovery reward that normally comes from defeating Moss Knight.
  await page.evaluate(() => {
  ...
  // Future real-care progression is represented by lifetime kept units.
  await page.evaluate(() => {
  ...
  check('new offspring starts young rather than instantly breedable', child.mature === false);
