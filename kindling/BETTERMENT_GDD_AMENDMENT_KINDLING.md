...
5. The companion moves to lineage history as a **Kindled ancestor**.
...
The Kindling rule gives lineage real meaning. Breeding creates continuity across generations rather than only cosmetic collection.

Potential future rule: a descendant of a Kindled ancestor has a small chance to inherit an **Ember Trait** derived from that ancestor.
...
- Do not silently kill the monster in the background; explicitly present the event on return.
- Test timezone, DST and 04:00 rollover edge cases before shipping.

## Still open

We can later test whether two days is the right grace period. Current design assumption: **2 consecutive fully missed care-days** because it creates stakes without making one rough day catastrophic.
