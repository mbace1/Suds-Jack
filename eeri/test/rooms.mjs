...
import { slugOf, parseSlug, PER_WORLD } from '../js/levelid.js?v=53';
import { ROOMS, LAB } from '../js/rooms.js?v=53';
import {
...
{
  const { PROPS, SCENERY, withDefaults } = await import('../js/scenery.js?v=53');
  const bad = [];
  for (const [world, rows] of Object.entries(SCENERY)) {
    rows.forEach((row, i) => {
      ...
      for (const [k, f] of Object.entries(PROPS[row.prop].fields)) {
        if (f.hex) {
          if (!/^#[0-9a-f]{6}$/i.test(p[k])) bad.push(`${where}: ${k} "${p[k]}" is not a #rrggbb colour`);
          continue;
        }
