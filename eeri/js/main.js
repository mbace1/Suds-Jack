...
import { slugOf, labelOf, parseSlug } from './levelid.js?v=1';
...
async function boot() {
  ...
  // screen, and a fragment naming a level that is not built YET (the game
  // grows three levels at a time) does the same: the address space is the
  // whole twelve from the start, the rooms arrive later.
  const fromHash = () => {
  ...
  // the address beside the name, so what is on screen matches what you can
  // paste to somebody: "1-2 · LEVEL 2 — THE SCAFFOLD"
  const setSiteName = () => {
  ...
  const HINT = {
    ...
    // fact. ◀ or ▶ depending on which side the machine is parked. (From the
    // design lineage's playtest — "a blocker that is not possible to jump
    // over" is what a lock feels like when the key is behind you.)
  ...
  // standing at a machine-shaped lock on foot: name the answer AND point at
  // it. Nothing here fires once the lock is cleared.
  function machineJob() {
    const d = site.def;
    if (!exc) return null;
