...
export const REACH = {
  ...
  // measures 4.85 — but that is 0.85 of a tile of margin, and DESIGN §4.1
  // locks "a full tile of slack" for a six-year-old. A 4-wide pit needed
  // full speed at the very lip; the playthrough bot, which never gives up,
  // stalled on one all afternoon. Three.
  gap: 3,
};
...
export function check(room) {
  ...
  // ---- the machine parks on the route, facing its job (DESIGN §8.0) -----
  // The playtest's sharpest finding: the lock-and-key shape — walk past the
  // machine, hit a wall you cannot jump, walk BACK, then a long dead drive —
  // "is the single biggest thing standing between this and a fun
  // platformer". So the shape is refused, not remembered: the machine may
  // park at most a short drive before its job, never far behind it, and
  // never past it. Fetching a carried load (the girder off its stack) is the
  // job itself, so the stack is exempt; the CLEARING obstacle is not.
  for (const o of r.obstacles) {
    if (!o.clears) continue;
    const m = r.machines.find((mm) => mm.verbs.includes(o.clears));
    if (!m) continue;
    const lead = o.at - m.x;                  // tiles of forward drive
    if (lead < 0) {
      note(`${r.name}: the ${m.type} parks at x=${m.x}, PAST the ${o.kind} it clears at x=${o.at} — `
        + 'you would meet the job before the key');
    } else if (lead > 8) {
      note(`${r.name}: the ${m.type} parks ${lead} tiles before the ${o.kind} at x=${o.at} — `
        + 'a walk-back-and-dead-drive is the lock-and-key shape §8.0 retired; park it on the route, facing the job');
    }
  }
  ...
  for (const o of r.obstacles) {
    ...
    const inTheWay = [];
    // the ball's danger is its ARC, not its pivot: inflate by the swing
    // radius, or a ball hung just outside the run still sweeps into it
    if (r.ball) {
      const swing = r.ball.len * 0.95 + 0.6;
      if (r.ball.px + swing > lo && r.ball.px - swing < hi) {
        inTheWay.push(`the swinging ball at x=${r.ball.px} (arc ±${swing.toFixed(1)})`);
      }
    }
