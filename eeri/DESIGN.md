...
the first level of world two — world and level, both 1-based, three levels
to a world exactly as §4.1 fixes. It is a URL: `/eeri/#eeri-1-2` opens that
level directly, which is what makes a level shareable for playtesting.
...
- **It is a NAMING layer over the flat site list, never a second list.** The
  ...
  "next level". Two lists that can disagree is the bug this project has
  already paid for more than once.
- **The address space is the whole twelve from the start.** The mapping is
  arithmetic, so a level gets its address the moment it is authored and
  `#eeri-2-1` is a link somebody can hold before world 2 exists. An address
  naming a level that is not built yet opens **1-1** rather than a black
  screen.
...
- **The HUD prints the address beside the name**, so what is on screen is
  what you can paste to somebody.
