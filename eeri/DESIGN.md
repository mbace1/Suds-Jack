...
## 0.2 What is BUILT, as of v11 — read this before planning work

The pivot in §0 is in the code now, not just in this file. Anything below
that is marked **built** should be extended rather than designed again.

| §  | thing | state |
|---|---|---|
| 2 | **stomp** | **built** — bounce 3.75 tiles, bigger than the jump on purpose |
| 2 | **climb** | **built** — ladders, `H` rungs, tops out on the deck, steps off with a direction |
| 3 | small enemies | **built** — `hopper` (timing), `roller` (spacing, not stompable), `skitter` (provocation) |
| 4 | four-beat levels | **built** — three levels, one idea each, beats marked in `js/rooms.js` |
| 4 | checkpoint | **built** — lights by being passed; a fall costs the middle of a level |
| 4.2 | flag, three phases, run past | **built** — small and big, `js/flag.js` |
...
| 5 | controller-first, glyph prompts | **built** — native pad in `input.js`, one glyph HINT set, touch buttons wearing the same glyphs |
| 2 | gizmos (planks, conveyors, hoists, hooks) | **not built** — the next cheapest variety, and §4's "one idea per level" means the kit IS the level count |
...
Crafted World is a KIT of materials (ART_TARGET §0.05): card, felt, painted
balsa, the cut flute of corrugated card. `craftMat(colour, material)` +
`craftBox(w, h, d, mat)` are the only way to make a surface — a prop built
with a bare `new THREE.MeshLambertMaterial` is **flat paint standing in a
crafted world**, and that is not a matter of taste: a probe of the live scene
once found three materials mapped and about seventy not, because every module
had grown its own little `M = (c) => …` helper. Site timber and steel are
`balsa`, cloth is `felt`, the earth and its cut faces are `flute`. The only
surfaces that stay bare are the ones craft.js names — a lit lamp, a pane of
glass, a shadow, a puff of smoke. **If you add a prop, this is the rule you
are most likely to break.**

**The rules of §4 are now a build step.** `check()` in `js/parts.js` refuses a
level that breaks them, and `node eeri/test/rooms.mjs` is where that fails.
It will reject: a ladder with no landing or no foot · no midway checkpoint ·
a **ride whose payoff sits in the first 45% of the room** (a ride is beat
3–4, not the way in) · a flag before the last obstacle · a bolt count that is
not 100 · fewer or more than three golden bolts · a golden bolt sitting on
the route where you would collect it by walking · a bolt hung where no jump
or ladder reaches it · a robot patrolling a deck that is not there.

So a new level is authored in `js/rooms.js` and **proved before it is
played**. If a rule here is wrong, change the rule in `parts.js` — do not
work around it in a room, because the next session will not know you did.
...
### 6.0 Drop points that are open and waiting (v7)

These exist in `assets/manifest.json` as **placeholders with their contract
already written down**, and the game is playing against a code-built stand-in
for each. Dropping the real thing in is a file plus a one-word status flip —
**no game code changes**, and the smoke gate checks the nodes on the way in.
That is the cheapest possible handover, so these are the ones to make first.

| manifest key | file | contracted nodes | what the game does with them |
|---|---|---|---|
| `pieces.flag` | `3d/flag_v1.glb` | `pole` `phase0` `phase1` `phase2` | shows exactly one phase; it BUILDS as Eeri arrives, so each phase must read at a glance mid-run |
...
| `pieces.checkpoint` | `3d/checkpoint_v1.glb` | `lamp` `cloth` | recolours `lamp` red → green and shows `cloth` when lit |

Still needed and **not yet given a drop point**, because their shape is the
asset producer's call rather than the game's: `hopper_v1.glb`,
`roller_v1.glb`, `bucket_v1.glb`. The behaviour is built and each one is
being played right now as a placeholder — so what they need is exactly what
§6.2 #1 says: a **`squash` node** (the game scales that one node when the
thing is stomped) and a **lit tell** that fires before it acts. Nothing else
about them is fixed. The roller is the odd one: it must read as *you cannot
land on this* from across the screen, because that is the rule the game plays
by — it is the one small thing a stomp does not kill.

Ladders and decks are **tiles**, not models — `js/level.js` draws the rungs —
so the scaffold kit in §6.2 #3 is now a look question rather than a blocker.
