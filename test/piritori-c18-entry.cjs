 ...
 try{
  for(const spec of [{name:'phone',width:412,height:915},{name:'landscape',width:844,height:390}]){
   ...
   try{
    ...
    assert.equal(entry.searchParams.get('release'),version);
    await tap(play);
    await page.waitForURL(url=>url.pathname===entry.pathname&&url.searchParams.get('release')===version,{timeout:60000});
    await idle();
    assert.equal(await page.locator('h1 span').textContent(),build);
    ...
    assert.ok((await about.locator('p').textContent()).startsWith(build+' —'));
