...
export const GAMES = [
  ...
  },
  {
    id: 'tokomove',
      ...
      note: 'tarkka HSL-verkko, viiden minuutin vuorot, oikea karttapohja, kameratasot, vaihtojen ajoitus ja maamerkit',
    },
    ja: {
      ...
      note: '正確なHSL網、5分シフト、実データの地面、3段階カメラ、乗換時間、ランドマーク',
    },
    status: 'active',
    live: true,
    note: 'exact HSL network geometry, five-minute shifts, real ground data, three camera scales, priced transfers, and a shift you can lose',
    title: 'Toko Move',
    tagline: 'Courier across a living Helsinki: read the fixed HSL network, catch moving trams and metro, transfer, walk, and deliver against the clock.',
    lineage: 'Mini Metro × real Helsinki × real-time tactics',
    tags: ['courier', 'transit', 'helsinki'],
    controls: 'pick a job · catch an arriving service · get off / transfer · walk to intercept',
    path: 'toko-move/',
    inRepo: true,
    accent: '#2f9fb8',
    art: 'tramstop',
  ...
  {
    id: 'flowsnow',
    score: { key: 'flowsnow.best', fmt: 'points' },
    fi: {
      tagline: 'Yksinkertainen lumilautapeli: yksi lasku aamunkoitosta hämärään, lauta uppoaa puuteriin ja nousee vauhdilla pintaan, lumi oikeana simulaationa.',
      lineage: 'Journey × Sword of the Sea × Shredders',
      controls: '← → kallista · ↑ kyyry · ↓ paino taakse: jarru kovalla, kellunta puuterissa · Väli ponnista · Z grab',
      note: 'v3 — lumipeitteellä on syvyys: lauta uppoaa ja nousee vauhdista pintaan, syvä puuteri hidastaa ja palkitsee, nokka voi upota — ja alastulo ei enää nyki',
    },
    ja: {
      tagline: 'シンプルなスノーボード。夜明けから夕暮れまでの一本の滑走、板はパウダーに沈み速度で浮き上がる、雪は本物の粒子シミュレーション。',
      lineage: 'Journey × Sword of the Sea × Shredders',
      controls: '← → 傾ける · ↑ タック · ↓ 後ろ荷重：硬い雪ではブレーキ、パウダーでは浮力 · スペース ジャンプ · Z グラブ',
      note: 'v3 — 積雪が深さを持った：板は沈み、速度で浮き上がる。深雪は遅いが高得点、ノーズは刺さる — そして着地でつっかえなくなった',
    },
    pad: 'native',
    status: 'active',
    note: 'v3 — the snowpack has depth: the board sinks in and planes back out with speed, deep powder is slow and pays, the nose can dig — and a landing no longer stutters',
    title: 'Flowsnow',
    tagline: 'A simplistic snowboarding game: one run from dawn to dusk, a board that sinks into the powder and planes back out of it, and snow that is a real particle simulation.',
    lineage: 'Journey × Sword of the Sea × Shredders',
    tags: ['snowboard', 'three.js', 'zen'],
    controls: '← → lean · ↑ tuck · ↓ weight back: brake on hardpack, float in powder · Space pop · Z grab',
    path: 'flowsnow/',
    inRepo: true,
    accent: '#f4a27a',
    art: 'flowsnow',
  },
  {
    ...
    score: { key: 'powderBest', fmt: 'points' },
      ...
      controls: 'vasen tatti ohjaa ja kaasuttaa · oikea tatti on painopiste ja kamera · WASD / nuolet · Väli tehostus · Shift spoileri · ohjain',
      note: 'ajettava kahdella alustalla — Blender-putki on auki laivoille ja maamerkeille',
    },
    ja: {
      tagline: '峡谷が切り裂く平原を走るロケットそり。タービンは遅れて回り、砂は沈んで動き、滑りは逆ハンドルで戻せる。',
      lineage: 'Wipeout × MotorStorm × スノーボード',
      controls: '左スティックで操舵と加速 · 右スティックは体重移動とカメラ · WASD / 矢印 · スペースでブースト · Shiftでスポイラー · パッド対応',
      note: '二つのシャシーで走れる — 船と地形物のBlenderパイプラインが開いた',
    },
    pad: 'native',
    status: 'active',
    note: 'drivable on two chassis — the Blender pipeline is open for ships and landmarks',
    title: 'Powder',
    tagline: 'A rocket sled on an open flatland cut by a canyon. The turbine spools late, the sand sinks and shifts under you, and a slide comes back with opposite lock.',
    lineage: 'Wipeout × MotorStorm × snowboarding',
    tags: ['racing', 'sim', 'three.js'],
  ...
  },
  {
    id: 'turf',
      ...
      note: 'v22 — useimmat korttelit PIDETÄÄN, ei tyhjennetä: kestä kierrokset loppuun. Viisi taistelua, kokemuspisteet, aseet ja pikkuesineet, palavat ruudut ja porraskuilut.',
    },
    ja: {
      tagline: '雨に濡れた北欧の裏路地でのグリッド戦術。敵の狙いは先に見え、遮蔽物が本当に効き、階段の吹き抜けへ突き飛ばす方が撃つより確実に倒せる。',
      lineage: 'Into the Breach × Metal Slug Tactics',
      controls: 'オペレーターをタップ、次にマスか敵をタップ · キーボード/パッドにも対応',
      note: 'v22 — 多くの区画は「制圧」ではなく「死守」— 規定ラウンドを耐え抜け。5つの戦闘、経験値・武器・装身具、燃える床と吹き抜け、独自に戦う敵。',
    },
    pad: 'native',
    status: 'active',
    note: 'v22 — most blocks are HELD, not cleared: outlast the round count. Five encounters, XP, drops and trinkets, burning tiles and open stairwells, enemies that each fight their own way.',
    title: 'TURF',
    tagline: 'Grid tactics on a rain-lit Nordic backlot: telegraphed hits, cover that actually matters, and a shove into an open stairwell that kills better than any gun.',
    lineage: 'Into the Breach × Metal Slug Tactics',
