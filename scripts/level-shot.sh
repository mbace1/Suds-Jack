#!/usr/bin/env bash
# level-shot.sh — the picture gate: the floor DRAWS a level's region, on both
# render paths, and the two pictures agree. (PR #447's gate, re-cut for this
# tree in v240: the level arrives as pendingLevel and startRun() plays it.)
#
...
if (location.hash === '#levelshot') {
  window._S = {
    state: () => gameState,
    level: () => pendingLevel && pendingLevel.id,
    isGpu: () => IS_GPU,
    begin: () => { startRun(); player._mercyT = 9e9; },   // startRun() routes to playLevel() while pendingLevel is armed
