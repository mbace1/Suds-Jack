...
no build step — open `index.html`. The rider is a **fat bird**, which at this size means
one round mass with no neck: the beak and the tail are what say it is a bird, and what
...
* **perfect** (< 0.26, a 15° window) — keep your speed, +6 %, **fever +1**
* **ok** (< 0.60) — lose a slice of speed proportional to the angle
...
* `terrain.js` — alternating crests and troughs joined by **raised-cosine** curves, so the
  slope is exactly zero at every extreme and a hill is poppable. Hills are generated from a
  **slope budget**: `steep` is the steepest point of the segment and the rise is derived
  from it by inverting the cosine, so the difficulty ramp (26° → 39° over 3.2 km) makes
  hills *bigger* and never sharper. The chain has a net **descent** (`DESCENT = 0.115`):
  without it the hills are a closed energy system and one bad climb parks you forever. The
  visible ribbon is a fixed-vertex sliding window rewritten each frame — no chunk pooling.
...
`node tiny2d/test/bench.cjs` drives the real `Skater` against the real `Terrain` at a fixed
timestep with no renderer (`test/sim.html`), rides five seeds three ways from five
distances, and reports what the terrain hands back: the share of landings that grade
perfect / ok / hard, the raw `align` ratios behind them, and the steepness under the
rider. `--sweep` does the same across candidate constants.

It exists because "is this fun to time?" has a measurable proxy. If the best line a bot can
ride never lands inside the perfect band, no player will either — which is exactly what was
happening before v4, at every distance. The curve it reports now:

| from | perfect | ok | hard | slope median / p90 / max |
|---|---|---|---|---|
| 0 m | 32% | 35% | 33% | 22° / 38° / 49° |
| 1600 m | 27% | 33% | 40% | 24° / 40° / 49° |
| 6000 m | 24% | 34% | 42% | 25° / 40° / 49° |

`node tiny2d/test/rider.html` (served, via `test/rider.html`) draws the bird large and at
the ~40px a phone gives it, which is the only honest way to check a silhouette.

## Known rough edges

* The bench measures a bot, not a person. The bands and the ramp want a human pass.
