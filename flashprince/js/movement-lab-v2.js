...
class World{
 ...
 draw(s){
   s.clear(C.VOID);
   s.rect(0,0,W,H,C.DARK);
   s.rect(0,0,W,44,C.VOID);
   for(let y=0;y<RH;y++)for(let x=0;x<RW;x++)if(this.solidTile(x,y)){
     s.rect(x*T,y*T,T,T,C.SOLID);
     if(!this.solidTile(x,y-1)){s.rect(x*T,y*T,T,2,C.EDGE);s.rect(x*T,y*T+2,T,1,C.NEAR)}
     if((x+y)%3===0)s.rect(x*T+2,y*T+6,6,1,C.FAR);
   }
   s.line(0,46,W,46,C.FAR,.45);
 }
}
class Lab{
