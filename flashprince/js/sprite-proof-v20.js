...
export class MotionStudy {
  ...
  paint() {
    ...
    s.text('FLASH PRINCE — CONRAD MOTION PASS v20', 8, 8, 4, 6);
    s.text(`START / RUN / STOP / TWO JUMPS — ${this.renderMode === 'upscale' ? '4X' : 'PIXEL'}`, 8, 18, 4, 6);
    s.present();
    if (this.renderMode === 'upscale') drawUpscaledSprite(s, frame, this.x, this.y, this.face, true);
  }

  snapshot() {
    return { version: 20, state: this.state, frame: this.frame, x: this.x, y: this.y, face: this.face, airMode: this.airMode, renderMode: this.renderMode };
  }
