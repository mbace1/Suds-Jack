...
class World{
 ...
 load(i){this.i=Math.max(0,Math.min(ROOMS.length-1,i));this.lab=ROOMS[this.i];this.g=this.lab.map.map(r=>[...r])}
 ...
 draw(s){
  ...
  s.veil([0,92,W,82,W,138,0,145],C.SUIT_HI,.06);for(let x=18;x<W;x+=53)vine(s,x,0,25+(x%18),C.EDGE);
  const m=this.lab.mark;
  if(m==='pod'){s.poly([18,158,34,149,53,153,57,164,48,169,25,168],C.SOLID);s.line(30,153,47,155,C.EDGE,1);s.disc(48,158,2,C.ALERT,.8)}
  if(m==='beacon'){s.line(198,127,244,118,C.EDGE,1);s.disc(245,118,2,C.ALERT,.8);s.line(245,118,257,109,C.FAR,.7)}
  if(m==='panel'){s.poly([252,91,269,85,278,91,272,101,255,101],C.SOLID);s.line(258,93,271,91,C.ALERT,.8)}
  if(m==='cable'){s.line(112,150,128,138,C.EDGE,1);s.line(128,138,144,143,C.EDGE,1);s.disc(128,138,2,C.ALERT,.8)}
  if(m==='door'){s.rect(260,64,28,80,C.SOLID);s.rect(264,68,20,72,C.DARK);s.line(270,74,270,132,C.EDGE,1);s.disc(278,84,2,C.ALERT,.8)}
...
class Game{
 constructor(){this.s=new Screen(document.getElementById('screen'));this.s.setPalette(paletteAt(2.15));this.input=new Input(this.s);this.world=new World();this.flash=0;this.room=0;this.spawn(false);addEventListener('keydown',e=>{if(/^Digit[1-5]$/.test(e.code))this.enter(Number(e.code.slice(-1))-1,1);if(e.code==='KeyR')this.spawn(false)});this.last=performance.now();this.acc=0;requestAnimationFrame(t=>this.frame(t))}
 ...
 step(){
  ...
  if(!this.hero.dead){
   ...
   if(this.hero.x>W+8&&this.room<ROOMS.length-1){this.enter(this.room+1,1);this.input.flush();return}
   if(this.hero.x<-8&&this.room>0){this.enter(this.room-1,-1);this.input.flush();return}
   if(this.hero.y>H+40)this.kill();
  }else if(this.input.anyPress)this.spawn(false);
  if(this.flash>0)this.flash--;this.input.flush();
 }
 paint(){const s=this.s;this.world.draw(s);drawCinematicFigureV5(s,this.hero,{far:C.SUIT,body:C.SUIT_HI,legs:C.SUIT,arms:C.SUIT_HI,skin:C.SKIN,hair:C.HAIR});if(this.flash)s.veil([0,0,W,0,W,H,0,H],C.ALERT,.12);s.present()}
