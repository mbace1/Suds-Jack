...
import {
  IDLE, STOP, STAND_JUMP, STAND_LAND, RUN_JUMP, RUN_LAND, STOP_HOLD,
...
const GROUND_Y = 169;
const START_N = 6;
// Locked to the approved v19/v17 cadence. Do not retime or redraw this run.
const RUN_HOLD = [5,4,3,3,4,3,3,4,5,4,3,3,4,3,3,4,5,4,3,3];
const RUN_SPEED = 1.22;
const LOCKED_RUN_SIGNATURE = 'ff0dd32f';
...
export class MotionStudy {
  ...
  paint() {
    ...
    const sourceFacesRight = this.state !== 'run' && this.state !== 'start';
    if (this.renderMode === 'classic') drawSprite(s, frame, this.x, this.y, this.face, sourceFacesRight);
    s.text('FLASH PRINCE — APPROVED RUN RESTORED v22', 8, 8, 4, 6);
    s.text(`LOCKED RUN / TURNS / JUMPS — ${this.renderMode === 'upscale' ? '4X' : 'PIXEL'}`, 8, 18, 4, 6);
    s.present();
    if (this.renderMode === 'upscale') drawUpscaledSprite(s, frame, this.x, this.y, this.face, sourceFacesRight);
  }

  snapshot() {
    return { version: 22, state: this.state, frame: this.frame, x: this.x, y: this.y, face: this.face, nextFace: this.nextFace, airMode: this.airMode, renderMode: this.renderMode, runSignature: LOCKED_RUN_SIGNATURE };
  }
