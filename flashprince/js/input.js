...
export class Input{
 constructor(scr){
  ...
  this.scheme='sticks';try{const s=localStorage.getItem('flashPrinceControls');if(s==='buttons'||s==='sticks')this.scheme=s}catch{}
 ...
 bindMenu(){const gear=document.getElementById('gear'),menu=document.getElementById('control-menu');gear?.addEventListener('click',e=>{e.stopPropagation();menu?.classList.toggle('open')});document.querySelectorAll('[data-scheme]').forEach(b=>b.addEventListener('click',()=>{this.scheme=b.dataset.scheme;try{localStorage.setItem('flashPrinceControls',this.scheme)}catch{}document.body.classList.toggle('buttons',this.scheme==='buttons');menu?.classList.remove('open');this.clear()}))}
 bindSticks(){
  if('ontouchstart'in window||matchMedia('(pointer:coarse)').matches)document.body.classList.add('touch');
  document.querySelectorAll('[data-stick]').forEach(el=>{const side=el.dataset.stick,s=this[side];const down=e=>{if(this.scheme!=='sticks')return;e.preventDefault();document.body.classList.add('touch');s.active=true;s.id=e.pointerId;s.ox=e.clientX;s.oy=e.clientY;s.dx=s.dy=0;el.setPointerCapture?.(e.pointerId);this.paintStick(s)};const move=e=>{if(!s.active||s.id!==e.pointerId)return;e.preventDefault();s.dx=e.clientX-s.ox;s.dy=e.clientY-s.oy;const m=Math.hypot(s.dx,s.dy);if(m>STICK_R){s.dx=s.dx/m*STICK_R;s.dy=s.dy/m*STICK_R}this.paintStick(s)};const up=e=>{if(s.id!==e.pointerId)return;e.preventDefault();s.active=false;s.id=null;s.dx=s.dy=0;this.paintStick(s)};el.addEventListener('pointerdown',down);el.addEventListener('pointermove',move);el.addEventListener('pointerup',up);el.addEventListener('pointercancel',up)})
 }
 ...
 poll(){
  ...
  const pads=navigator.getGamepads?navigator.getGamepads():[];for(const p of pads){if(!p)continue;const ax=p.axes[0]??0,ay=p.axes[1]??0,ry=p.axes[3]??0;L=L||ax<-.4||p.buttons[14]?.pressed;R=R||ax>.4||p.buttons[15]?.pressed;U=U||ay<-.5||p.buttons[12]?.pressed;D=D||ay>.5||p.buttons[13]?.pressed;J=J||ry<-.35||p.buttons[0]?.pressed;F=F||p.buttons[2]?.pressed||p.buttons[7]?.pressed||p.buttons[5]?.pressed;G=G||p.buttons[3]?.pressed||p.buttons[1]?.pressed;if(p.buttons[9]?.pressed&&!this.padPrev[9])this.pausePress=true;this.padPrev=p.buttons.map(b=>b.pressed);break}
