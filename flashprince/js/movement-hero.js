...
const FLOW = {
  ...
  // Standing and running jumps now have different preparation silhouettes.
...
export class MovementHero extends Hero {
  ...
  // A one-tile obstacle is not a jump problem. If the feet meet a 16px-ish
  // block while head/chest space is clear, the body steps onto it.
  canLowMantle(world) {
  ...
  lowMantleFrame() {
    ...
    // Feet gain exactly one tile while the root crosses onto the block.
    this.y = this.lowStartY - 16 * (t * t * (3 - 2 * t));
  }

  // Find a nearby floor lip for deliberate climb-down. This only triggers when
  // DOWN is held at an actual edge; it never turns a normal crouch into a drop.
  dropLip(world) {
    for (let d = 4; d <= 12; d++) {
  ...
  canShimmy(world, dir) {
    ...
    // Keep the hands on a continuous exposed top surface.
  ...
  // Running landings keep their own recovery instead of always collapsing to
  // the standing landing. The inherited air collision remains authoritative.
