...
export class MotionStudy {
  ...
  paint() {
    ...
    s.text('FLASH PRINCE — v18 RUN RESTORED / v24', 8, 8, 4, 6);
    s.text(`V23 ART REMOVED / RUN LOCKED — ${this.renderMode === 'upscale' ? '4X' : 'PIXEL'}`, 8, 18, 4, 6);
    s.present();
    if (this.renderMode === 'upscale') drawUpscaledSprite(s, frame, this.x, this.y, this.face, sourceFacesRight);
  }

  snapshot() {
    return { version: 24, state: this.state, frame: this.frame, x: this.x, y: this.y, face: this.face, nextFace: this.nextFace, airMode: this.airMode, renderMode: this.renderMode, runSignature: LOCKED_RUN_SIGNATURE };
  }
