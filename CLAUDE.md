...
Traps worth remembering, all found by testing or play:

* **Airborne flicks need arming too.** The grounded gesture is gated by `loaded`, but the
  air branch had no equivalent, so holding a flick out re-fired a trick every time the
  0.12 s cooldown lapsed. Arming fixes it — but the re-arm check must run *even while
  cooling*, because the return stroke of a flick lands inside the cooldown window, and
  gating it leaves the stick permanently disarmed so the second flick of a left-right pair
  never fires.
* **A fixed vertical FOV crops narrow screens.** `PerspectiveCamera.fov` is vertical, so
  4:3 sees ~86° horizontally where 16:9 sees ~102°. Keep a horizontal minimum instead.
* **A landscape phone is ~390 px tall** — the menu overflowed and the start button sat
  off-screen, so the game could not be started at all on one.
