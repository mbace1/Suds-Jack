...
import { bodySystem,                        } from './body.js?v=1';
import { Manifold } from './manifold.js?v=1';
import {
  addScaled3,
  clamp,
  diagonal,
  ...
  step()       {
    ...
    for (let i = bodies.length - 1; i >= 0; i--) {
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        // HD DEVIATION (the one change to the port): two static bodies never need a
        // contact. Hyper Dagger puts a resting gib to SLEEP by making it static, so a
        // settled pile costs a sphere test per pair instead of a collide and a solve.
    ...
    for (const force of this.forces) {
      // HD DEVIATION, continued: a contact between two bodies that have both gone to
      // sleep since it was made is dropped rather than re-collided every step
      if (force instanceof Manifold && force.bodyA?.mass <= 0 && force.bodyB.mass <= 0) { force.unlinkFromBodies(); continue; }
...
const st = vec3();
const scaleTmp = (v    , s        )     => scale3(st, v, s);
