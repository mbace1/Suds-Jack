...
function faceAxes(box     , axisIndex        )                                                     {
...
function buildIncidentFace(box     , axisIndex        , referenceNormal    )       {
...
function clipPolygonAgainstPlane(inVerts      , planeNormal    , planeOffset        )       {
  const out       = [];
...
function testAxis(boxA     , boxB     , delta    , axis    , type        , indexA        , indexB        , best         )          {
...
function supportEdge(box     , axisIndex        , dir    )           {
...
function buildEdgeContact(bodyA       , bodyB       , boxA     , boxB     , axisA        , axisB        , normalAB    )               {
 ...
 */
export function collide(bodyA       , bodyB       , basisOut    )               {
  const boxA = makeOBB(bodyA);
  const boxB = makeOBB(bodyB);
  const delta = sub3(vec3(), boxB.center, boxA.center);

  const bestFace          = { type: AXIS_FACE_A, indexA: 0, indexB: 0, separation: -FLT_MAX, normalAB: vec3(), valid: false };
  const bestEdge          = { type: AXIS_EDGE, indexA: 0, indexB: 0, separation: -FLT_MAX, normalAB: vec3(), valid: false };
