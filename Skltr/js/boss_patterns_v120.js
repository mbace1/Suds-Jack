...
Enemy.prototype.update=function(dt,player,pool,heightAt=()=>0,bound=47,...rest){
  ...
  if(this.type==='boss'){
    if(mode===0){
      // Ring with two deliberate safe gaps.
      ...
      // Aimed wedge leaves lateral escape routes.
      ...
      // Slow rotating spokes force circling rather than panic-dodging.
  ...
  if(this.type==='boss2'){
    if(mode===0){
      ...
      // One slow homing shot supported by two non-homing escorts.
      ...
      // Cross-lane burst: readable cardinal rails with rotating offset.
...
// Small, non-modal cue so boss attacks can be learned while moving.
