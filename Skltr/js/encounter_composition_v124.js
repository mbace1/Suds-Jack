...
Enemy.prototype.update=function(dt,player,pool,heightAt=()=>0,bound=47,...rest){
  ...
  if(this.type==='chaser'){
    // In mixed fights, HOUNDs bias toward the player's exposed side rather than stacking directly in front.
    ...
    // When a player is already being held by lane fire, commit harder to force the dash decision.
    if((tort||mixed)&&d<9&&d>2.2){this.x+=nx*dt*1.8;this.z+=nz*dt*1.8;}
  }

  if(this.type==='turret'){
    // TORTOISEs offset from one another's firing axis so multiple anchors create crossing lanes, not one blob.
    ...
    if((tort||mixed)&&d>12&&d<28&&this._v124LaneT%5.8>3.8){
      ...
      const cx=this.x+tx*side*dt*1.2,cz=this.z+tz*side*dt*1.2;
  ...
  if(this.type==='flyer'){
    // WASPs react to nearby cover use by climbing and shifting laterally, trying to restore a different attack angle.
    const api=window._skltrBVH96;
    ...
    if((wasp||mixed)&&!los){
      ...
      this.x+=tx*side*dt*1.8;this.z+=tz*side*dt*1.8;
    }
    // In late mixed encounters, stay a touch farther out so missiles and ladders overlap HOUND pressure cleanly.
...
// Small role cue for the first second of authored mixed encounters.
