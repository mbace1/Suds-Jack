...
// SKLTR v105-v108 — authored hero architecture that is also gameplay geometry.
...
// existing three-mesh-bvh layer. No visual-only collision mismatch.
...
function build(k){if(!scene)return;const base=scene.getObjectByName('SKLTR_V60_WORLD');if(!base){requestAnimationFrame(()=>build(k));return}const old=base.getObjectByName('SKLTR_V108_HERO');if(old)base.remove(old);const r=new THREE.Group();r.name='SKLTR_V108_HERO';base.add(r);const p=PAL[k];
 if(k==='wire'){
   ...
   box(r,0,0,37,18,2.4,3.2,p);box(r,-35,0,-4,4,10,4,p);box(r,35,0,6,4,12,4,p);
 }else if(k==='brutal'){
   aperture(r,-17,-6,18,8,3.6,p,.08);aperture(r,19,16,16,10,4.2,p,-.14);
   box(r,-31,0,29,10,13,8,p,.10);box(r,32,0,-26,12,17,9,p,-.08);box(r,0,0,-35,24,3.2,5,p);
 }else if(k==='bio'){
   for(const a of[-.7,0,.7]){const x=Math.sin(a)*29,z=Math.cos(a)*25-5;arch(r,x,z,11,11,1.25,p,a)}
   for(const x of[-31,31]){const q=mesh(r,new THREE.CylinderGeometry(2.4,3.7,15,7),x,7.5,20,p);q.rotation.z=x<0?.14:-.14}
 }else if(k==='physical'){
   arch(r,-24,-18,17,10,2,p,.12);arch(r,25,20,20,12,2.2,p,-.12);
   box(r,0,0,-34,30,4,6,p);box(r,-37,0,9,7,16,9,p);box(r,38,0,-5,8,19,9,p);
 }else{
   ...
   aperture(r,0,-24,15,9,4,p,0);aperture(r,0,25,15,9,4,p,Math.PI);
 }
