...
// The warehouse GLB replaces the procedural room on desktop. The Blender
// skater and board are an OPTION since v4 — the code-built THPS figure is the
// house skater on both tiers — so they load only when asked, and switching
// between the two is a visibility flip once both exist.
export function loadArt({scene,rider,deck,body,environment,renderer,host,skater='thps'}){
 ...
 const quality=mobile?'mobile':'desktop',loader=new GLTFLoader();let hipRest=null,mixer=null,clips={},current='',model=null,boardModel=null,disposed=false,wasAir=false,landing=0,manifest=null,shown='thps';
 ...
 const footRest={};const resources=[];const extraTextures=[];const loaded={};
 ...
 function load(name){
  if(!loaded[name])loaded[name]=(async()=>{
   const gltf=await loader.loadAsync(new URL(manifest.models[name][quality],base).href);if(disposed){gltf.scene.traverse(disposeObject);return null}
   ...
   if(name==='skater'){model=gltf.scene;model.visible=false;rider.add(model);mixer=new T.AnimationMixer(model);for(const clip of gltf.animations){const name=clip.name.split('|').pop();clips[name]=mixer.clipAction(clip)}hipRest=model.getObjectByName('hips')?.position.clone();model.updateWorldMatrix(true,true);for(const side of ['l','r']){const foot=model.getObjectByName('foot_'+side);footRest[side]=rider.getWorldQuaternion(new T.Quaternion()).invert().multiply(foot.getWorldQuaternion(new T.Quaternion()))}host.dataset.clips=Object.keys(clips).join(',')}
   if(name==='board'){boardModel=gltf.scene;boardModel.visible=false;deck.add(boardModel)}
   return gltf;
  })().catch(e=>{delete loaded[name];throw e});
  return loaded[name];
 }
 function show(kind){
  shown=kind==='blender'&&model?'blender':'thps';
  if(model)model.visible=shown==='blender';
  body.visible=shown!=='blender';
  for(const child of deck.children)child.visible=child===boardModel?shown==='blender':shown!=='blender';
  host.dataset.skater=shown;
 }
 const ready=fetch(new URL('manifest.json?v=2',base)).then(r=>{if(!r.ok)throw Error('Art manifest unavailable');return r.json()}).then(async m=>{
  manifest=m;
  // Some mobile GPUs fail only after a GLB is inserted, turning the complete
  // framebuffer white. The procedural warehouse and the code-built skater are
  // the compatibility tier; desktop gets the Blender warehouse, and the
  // Blender skater when the option asks for it.
  const models=mobile?[]:['warehouse',...(skater==='blender'?['skater','board']:[])];
  await Promise.all(models.map(load));show(skater);host.dataset.art='ready';return true;
 }).catch(e=>{host.dataset.art='fallback';show('thps');console.warn('CONCRETE: using available procedural art.',e);return false});
 async function setSkater(kind){
  await ready.catch(()=>false);
  if(kind==='blender'&&!mobile&&manifest&&!disposed){try{await Promise.all([load('skater'),load('board')])}catch(e){console.warn('CONCRETE: Blender skater unavailable.',e)}}
  show(kind);return shown;
 }
 ...
 function update(dt,s){if(!mixer||!model||shown!=='blender')return;if(wasAir&&!s.air)landing=.2;wasAir=s.air;landing=Math.max(0,landing-dt);
 ...
 return {ready,update,mobile,setSkater,get skater(){return shown},dispose(){disposed=true;mixer?.stopAllAction();for(const t of extraTextures)t.dispose();for(const r of resources){r.removeFromParent();r.traverse(disposeObject)}}};
}
