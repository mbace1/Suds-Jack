...
(async () => {
  ...
  for (const f of files) {
    ...
    const m = await pg.evaluate(async ({ url, slice, src }) => {
      ...
      return measure(g.getImageData(0, 0, W, H).data, W, H, slice);
    }, { url: `data:image/png;base64,${b64}`, slice: HEAD_SLICE, src: MEASURE_SRC });
    rows.push(m);
