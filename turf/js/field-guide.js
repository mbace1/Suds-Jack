...
export function fieldGuide(state, abilities = []) {
  ...
  if (state.armedAbility) {
    const ability = abilities.find(a => a.id === state.armedAbility);
    const name = ability?.name || 'Ability';
    if (ability?.shape === 'self' || ability?.shape === 'adjacent-all') return `${name} ready. Press Use below or tap your operator. Cancel backs out.`;
    return `${name} ready. Choose a highlighted target${ability?.shape === 'empty-tile' ? ' tile on the board' : ' on the board or below'}. Cancel backs out.`;
  }
  ...
  if (state.forecasts?.size) return '3 · Choose an attack target below or tap a rival on the board. Odds and damage appear before you commit.';
